	...
	$.extend({
		...
		form: {
			...
			reset: function(formId) {
				...
				//  $("#marketId").val("").trigger('change');
				//  $("#marketType").val("").trigger('change');
//				 $("#type").val("").trigger('change');
		...
		modal: {
			...
			// 弹出层指定宽度,无按钮
			openNoButton: function (title, url, width, height) {
				//如果是移动端，就使用自适应大小弹窗
		...
		operate: {
			...
			//修改状态
			changeChannel:function(channelId,status){
				var tex=status==1?"禁用":"启用";
				var cstatus=status==1?0:1;
				$.modal.confirm("确定"+tex+"该条" + $.table._option.modalName + "信息吗？", function() {
			...
			//修改appswitch状态
			changeSwitch:function(id,status){
				var tex=status==1?"等待审核":"正式运营";
				var cStatus=status==1?0:1;
				$.modal.confirm("确定切换该条" + $.table._option.modalName + "信息状态"+tex+"吗？", function() {
					var url= $.table._option.changeUrl;
					var data = {
						"id": id ,
						"status":cStatus
